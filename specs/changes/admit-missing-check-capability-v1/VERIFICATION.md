...
| `REQ-MCHECK-009` | Covered | Change status is `Implemented`; support wording remains gated until final verification. |
...
| `REQ-MCHECK-024` | Covered | Envelope action Schema is the closed `revise`, `execute`, `replay`, `stop` subset. |
...
| `REQ-MCHECK-028` | Covered | Decision tests bind every hint to a compact observation or witness evidence ref. |
...
| `REQ-MCHECK-039` | Covered | Timeout, cancellation, unavailable Analyzer, incomplete scope, and completed unknown have distinct statuses/codes. |
...
| `REQ-MCHECK-045` | Covered | Provenance distinguishes `real_analyzer` from `test_double`; test-double execution is capped at `generated`. |
| `REQ-MCHECK-046` | Covered | Compact result independently stores operation status, decision, and verification level. |
| `REQ-MCHECK-047` | Covered | Portable evidence binds exact completeness boundary and Analyzer/adapter versions. |
...
The unit/integration suite passed 27 files and 154 tests. The real Golden used
