...
- Verification conclusion: the architecture requirements are Verified by the listed deterministic tests and package checks. They are merged into root `SPEC.md` version 1.1 and this change is Archived. Flow itself remains `Implemented`, not `Verified`, under `introduce-flow-capability-v1`; it MUST NOT be claimed as supported.
