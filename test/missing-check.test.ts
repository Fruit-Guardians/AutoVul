...
const hypothesis = {
  ...
  scope: { kind: "single_file_cfg", file: "extensions/msteams/src/monitor-handler.ts", entry: "registerMSTeamsHandlers callback" },
} as const;
...
function observation(overrides: Partial<MissingCheckAnalyzerObservation> = {}): MissingCheckAnalyzerObservation {
  return {
    ...
    capability_gaps: [], evidence_refs: ["missing-check/vulnerable/unchecked.sarif"], analyzer: { analyzer_id: "codeql", available: true }, ...overrides,
  };
}
...
describe("MissingCheck v1 contracts and policy", () => {
  ...
  it("requires a persisted unchecked witness before emitting check_missing", () => {
    const result = decideMissingCheck(observation({ relation: { state: "unchecked_witness", unchecked_witnesses: [{ operation: { file: "handler.ts", start_line: 10 }, evidence_ref: "witness.sarif" }], checked_witnesses: [] } }), "reproduce");
  ...
  it("does not turn a missing check selector into absence proof", () => {
    const result = decideMissingCheck(observation({ required_check: { state: "not_found", locations: [] } }), "reproduce");
  ...
  it("requires fixed checked evidence before raising differential", () => {
    ...
    const noFixed = decideMissingCheck(observation({ relation: vulnerable, fixed_relation: { state: "not_run", unchecked_witnesses: [], checked_witnesses: [] } }), "differential");
    expect(noFixed.verificationLevel).toBe("reproduced");
    const fixed = decideMissingCheck(observation({ relation: vulnerable, fixed_relation: { state: "checked_witness", unchecked_witnesses: [], checked_witnesses: [{ operation: { file: "handler.ts", start_line: 10 }, evidence_ref: "f.sarif" }] } }), "differential");
  ...
  it("keeps capability gaps distinct from a completed check decision", () => {
    const result = decideMissingCheck(observation({ capability_gaps: [{ code: "MCHECK_ALIASING_UNSUPPORTED", path: "/scope" }] }), "reproduce");
    expect(result.decision.outcome).toBe("unknown");
  ...
  it("uses the aggregate research and run APIs without Flow routing", async () => {
    const port: MissingCheckExecutionPort = { async execute(_request: MissingCheckExecutionRequest): Promise<MissingCheckAnalyzerObservation> {
      return observation({ relation: { state: "unchecked_witness", unchecked_witnesses: [{ operation: { file: "handler.ts", start_line: 10 }, evidence_ref: "v.sarif" }], checked_witnesses: [] }, fixed_relation: { state: "checked_witness", unchecked_witnesses: [], checked_witnesses: [{ operation: { file: "handler.ts", start_line: 10 }, evidence_ref: "f.sarif" }] } });
    } };
    const artifacts = new MemoryArtifactStore();
    const app = new Application({ codeql: new FakeCodeqlPort(), artifacts, clock: new FixedClock(), ids: new FixedIdGenerator("run_mcheck"), missingCheck: port });
    ...
    expect(result).toMatchObject({ capability: "missing_check", operation_status: "completed", verification_level: "differential", decision: { outcome: "check_missing", fixed_outcome: "check_present" } });
    ...
    expect(replayed).toMatchObject({ capability: "missing_check", verification_level: "differential" });
    await app.close();
  });
